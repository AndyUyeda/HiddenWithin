...
class MEStringSearcher : NSObject {
    ...
    func getStringWithLeftBounds(lft :NSString, rght : NSString) ->NSString?{
        ...
        leftRange = string.rangeOfString(lft as String, options: NSStringCompareOptions.CaseInsensitiveSearch, range: range)
        if(!validRange(leftRange,len:length)){
            return nil
        }
        leftSum = (leftRange.location + leftRange.length) as Int
        
        //print(leftSum < length)
        if(leftSum > length){
            return nil
        }
        rightRange = string.rangeOfString(rght as String, options: NSStringCompareOptions.CaseInsensitiveSearch, range: NSMakeRange(leftSum,length - leftSum))
    ...
    func moveToString(str : NSString){
        var tempR : NSRange!
        tempR = string.rangeOfString(str as String, options: NSStringCompareOptions.CaseInsensitiveSearch, range: range)
